...
echo 'Installing the php-kata-scaffolding ...'
...
git clone https://github.com/Chemaclass/php-kata-scaffolding "$projectName"
cd "$projectName" || exit
...
find . -type f -exec \
  sed -i '' -e "s/php-kata-scaffolding/$projectName/g" {} +
find . -type f -exec \
  sed -i '' -e "s/php-kata-scaffolding/$containerName/g" {} +
